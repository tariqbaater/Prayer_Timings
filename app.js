...
Isha: ${today.times.Isha}`;
  ...
  $("downloadPdf").addEventListener("click", async () => {
    ...
    try {
      ...
      doc.setFillColor(238, 235, 227);
      doc.rect(0, 0, 297, 210, "F");

      // Header
      doc.setFont("helvetica", "bold");
      doc.setFontSize(36);
      doc.setTextColor(43, 87, 70);
      doc.text("أوقات الصلاة", 148.5, 25, { align: "center" });

      doc.setFontSize(18);
      doc.text("ADHAN TIMINGS", 148.5, 38, { align: "center" });

      // City and date info
      doc.setFontSize(12);
      doc.setTextColor(60, 60, 60);
      doc.text(city, 148.5, 52, { align: "center" });
      doc.text(hijri, 148.5, 60, { align: "center" });
      ...
      // Table header
      doc.setFillColor(43, 87, 70);
      doc.rect(tableX, tableY, colWidth * 6, rowHeight, "F");
      doc.setTextColor(255, 255, 255);
      doc.setFontSize(12);
      doc.text("Date", tableX + 3, tableY + 10);
      doc.text("Fajr", tableX + colWidth + 3, tableY + 10);
      doc.text("Sunrise", tableX + colWidth * 2 + 3, tableY + 10);
      doc.text("Dhuhr", tableX + colWidth * 3 + 3, tableY + 10);
      doc.text("Asr", tableX + colWidth * 4 + 3, tableY + 10);
      doc.text("Maghrib", tableX + colWidth * 5 + 3, tableY + 10);
      ...
      rows.forEach((r, i) => {
        ...
        if (isToday) {
          doc.setFillColor(43, 87, 70);
          doc.setTextColor(255, 255, 255);
          doc.rect(tableX, y, colWidth * 6, rowHeight, "F");
        } else if (i % 2 === 1) {
        ...
        const dateCell = r.date.toLocaleDateString("en-US", { month: "short", day: "numeric" });
        doc.text(dateCell, tableX + 3, y + 10);
        doc.text(r.times.Fajr, tableX + colWidth + 3, y + 10);
        doc.text(r.times.Sunrise, tableX + colWidth * 2 + 3, y + 10);
        doc.text(r.times.Dhuhr, tableX + colWidth * 3 + 3, y + 10);
        doc.text(r.times.Asr, tableX + colWidth * 4 + 3, y + 10);
        doc.text(r.times.Maghrib, tableX + colWidth * 5 + 3, y + 10);
      });

      // Footer
      doc.setFontSize(8);
      doc.setTextColor(120, 120, 120);
      doc.text("Generated by Adhan Timings", 148.5, 200, { align: "center" });

      doc.save(`prayer-times-${city.replace(/\s+/g, "-").toLowerCase()}.pdf`);
    } catch (err) {
